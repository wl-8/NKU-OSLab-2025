...
* **Lab5**：并没有显示的`sched_init()`函数（因为 Lab5 调度器直接遍历进程列表）。
...
如果要添加一个新的调度算法（例如 Stride 调度算法），主要需要进行以下三个步骤的修改：

1. **实现具体的调度策略**：
   创建一个新的 C 文件（如 `kern/schedule/default_sched_stride.c`），并在其中实现 `sched_class` 接口要求的 5 个核心函数：

   *   `init`: 初始化运行队列（如初始化斜堆）。
   *   `enqueue`: 将进程加入运行队列（如插入斜堆）。
   *   `dequeue`: 将进程移出运行队列（如从斜堆删除）。
   *   `pick_next`: 选择下一个要运行的进程（如选择 stride 最小的进程）。
   *   `proc_tick`: 处理时钟中断（更新 stride 或时间片）。

2. **定义调度类实例**：
   在上述文件中，定义一个 `struct sched_class` 类型的全局变量（如 `stride_sched_class`），并将步骤 1 中实现的函数赋值给对应的函数指针。
...
3. **在初始化时进行绑定**：
   修改 `kern/schedule/sched.c` 中的 `sched_init` 函数，将全局指针 `sched_class` 指向新定义的调度类实例。

   ```c
   void sched_init(void) {
       // ...
       sched_class = &stride_sched_class; // 切换为 Stride 调度器
       // ...
   }
   ```
...
当前的设计采用了**面向对象的多态思想**和**机制与策略分离**的原则，使得切换调度算法变得非常容易：

1.  **接口标准化**：
    `sched_class` 定义了一套标准的接口规范。内核的其他部分（如进程创建、退出、等待等机制）只依赖于这个接口，而不依赖于具体的实现。这意味着无论底层算法是 RR 还是 Stride，上层逻辑都不需要改变。

2.  **运行时绑定**：
    调度算法的选择仅仅取决于全局指针 `sched_class` 指向哪个实例。这意味着切换算法只需要修改一行代码（指针赋值），甚至可以在运行时动态切换（如果实现了相应的状态迁移逻辑）。
