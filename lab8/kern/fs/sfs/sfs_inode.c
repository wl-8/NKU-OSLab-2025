...
/*  
 * sfs_io_nolock - 读写文件内容，将文件从offset位置到offset+length的数据在磁盘块与内存缓冲区之间传输
 * @sfs:      SFS文件系统实例
 * @sin:      内存中的SFS inode结构
 * @buf:      读写数据的内存缓冲区
 * @offset:   文件内的起始偏移量
 * @alenp:    输入为请求读写的长度（指针），输出为实际读写的长度
 * @write:    布尔值，0表示读操作，1表示写操作
 */
static int
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    *alenp = 0;
    ...
    if (endpos > SFS_MAX_FILE_SIZE) {
        endpos = SFS_MAX_FILE_SIZE;  // 限制在最大文件大小内
    }
    
    // 对于读操作，需要检查文件实际大小
    if (!write) {
        if (offset >= din->size) {
            return 0;  // 偏移量超出文件大小，无数据可读
        }
        if (endpos > din->size) {
            endpos = din->size;  // 限制在文件实际大小内
        }
    }
    ...
    uint32_t nblks = endpos / SFS_BLKSIZE - blkno;  // 需要处理的完整块数量

  // LAB8:EXERCISE1 2313857
    /*
     * 文件I/O分为三种情况处理：
     * (1) 首块部分读写：当offset不是块对齐时
     * (2) 中间完整块读写：可以整块读写的对齐块
     * (3) 末块部分读写：当结束位置不是块对齐时
     */
    ...
    // ===== 情况1：处理首个非对齐块 =====
    // 如果offset不是块对齐的，需要从blkoff处读写到首块末尾
    if (blkoff != 0) {
        // 计算首块需要读写的字节数
        // 如果还有后续块，读到块末尾；否则只读到endpos
...
out:
    *alenp = alen;  // 返回实际读写的字节数
    // 如果是写操作且扩展了文件大小，更新inode中的文件大小
    if (offset + alen > sin->din->size) {
        sin->din->size = offset + alen;
        sin->dirty = 1;  // 标记inode已修改，需要写回磁盘
    }
    return ret;
}
