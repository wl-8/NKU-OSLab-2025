               ...
               bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            assert(npage != NULL);
            int ret = 0;
            /* LAB5: Completed - Copy memory content and build page mapping
             * replicate content of page to npage, build the map of phy addr of
             * nage with the linear addr start
             */
            // (1) Find src_kvaddr: the kernel virtual address of source page
            void *src_kvaddr = page2kva(page);
            // (2) Find dst_kvaddr: the kernel virtual address of destination page
            void *dst_kvaddr = page2kva(npage);
            // (3) Copy memory from src_kvaddr to dst_kvaddr, size is PGSIZE
            memcpy(dst_kvaddr, src_kvaddr, PGSIZE);
            // (4) Build the map of physical address of npage with the linear address start
